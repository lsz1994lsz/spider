...
class WalianKuaixunPipeline(object):
    def __init__(self):
        self.list = []
        self.db = test_db
        self.db.connect()
    ...
    def __save_mysql(self, li):
        ...
        cur = self.db.cursor()
        sql = "select count(id) from walian_kuaixun where title = %s;"
        ...
        if result[0] != 0:
            ...
            with self.db.atomic():
                ...
                walian_kuaixun.insert(title = li['title'] ,text = li['text'], record_timestamp = record_timestamp).execute()
